...
(function() {
    ...
    function isReady() {
        var ready = true;
        //number of objects in resourceCache
        var size = Object.keys(resourceCache).length;
        var x = size;
        for(var k in resourceCache) {
            if(resourceCache.hasOwnProperty(k) &&
               !resourceCache[k]) {
                ready = false;
                //decrement if not ready
                var x = x - 1;
            }
        }
        //calculate percentage ready
        document.getElementById('percent').innerHTML = Math.round((x / size) * 100) + '%';
        return ready;
    }
